...
describe('expandSpmDependencies (reserved React Native names)', () => {
  ...
  it('throws on a case-insensitive match against a reserved name', () => {
    expect(() =>
      expand([{name: 'some-lib', root: '/s', platforms: {ios: {}}}], {
  ...
  it('throws SpmNameCollisionError, so callers can tell a misconfiguration from a resolution failure', () => {
    expect(() =>
      expand([{name: 'some-lib', root: '/s', platforms: {ios: {}}}], {
        '/s': {spm: {name: 'ReactHeaders'}},
      }),
    ).toThrow(SpmNameCollisionError);
  });
  ...
  it('rejects the codegen package products declared by the codegen template', () => {
    for (const product of ['ReactCodegen', 'ReactAppDependencyProvider']) {
      expect(() =>
        expand([{name: 'some-lib', root: '/s', platforms: {ios: {}}}], {
          '/s': {spm: {name: product}},
        }),
