...
describe('main() — autolinking plugin host exemption', () => {
  ...
  function buildFixture({withPlugin, depName = 'expo', extraDepName = null}) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        dependencies: {
          ...
          ...(extraDepName != null
            ? {[extraDepName]: {root: expoDir, platforms: {ios: {}}}}
            : {}),
        },
      }),
    );
  ...
  it('reports the reserved-name diagnosis, not the generic dep-vs-dep one, when two deps take the same reserved name', () => {
    // Guards the check order: the reserved diagnosis is the specific one.
    const {appRoot, rnRoot} = buildFixture({
      withPlugin: false,
      depName: 'react-headers',
      extraDepName: 'react_headers',
    });
    expect(() =>
      main(['--app-root', appRoot, '--react-native-root', rnRoot]),
...
describe('main() — spm.modules names', () => {
  ...
  it('rejects a reserved product name in any casing', () => {
    ...
    expect(() => run(app)).toThrow(SpmNameCollisionError);
    expect(() => run(app)).toThrow(/ReactHeaders/);
  });
