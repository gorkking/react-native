...
describe('generateXCFrameworksPackageSwift derives from the shared name constants', () => {
  ...
  it('never emits an "undefined" dependency, whatever the product list holds', () => {
    jest.isolateModules(() => {
      jest.doMock('../spm-utils', () => {
        const actual = jest.requireActual('../spm-utils');
        return {
          ...actual,
          // A one-entry list is what makes a positional `[0]` lookup produce
      ...
      expect(generate()).not.toContain('undefined');
    });
  });
